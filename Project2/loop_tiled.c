...
void magn_theta_calc(){
  ...
  for (i = 0; i<N;i+=8) {//fissioned loop
    for (j = 0; j<M; j+=8){ //blocksize=32
      for (z = i; z<i+8;z++){
        for (w = j; w<j+8;w++){
    ...
    //determine edge direction and color accordingly
	     if (current_y[z][w] > 45){//authaireto orio,45 gia car,30 gia cat,30 gia sunflower
	        if (theta[z][w] >= 0 && theta[z][w] < 22.5){ //horizontal edge
                  ...
                  current_v[z][w] = 107;}
          else if (theta[z][w] >= 157.5 && theta[z][w] <= 180) {//horizontal edge
      	          current_u[z][w] = 255;//blue UV channel values
                  current_v[z][w] = 107;}
          else if (theta[z][w] >= 22.5 && theta[z][w] < 67.5) {//inclined edge
                  current_u[z][w] = 84;//red UV channel values
                  current_v[z][w] = 255;}
          else if (theta[z][w] >= 67.5 && theta[z][w] < 112.5) {//vertical edge
